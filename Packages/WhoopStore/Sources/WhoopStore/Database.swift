...
extension WhoopStore {
    ...
    static func makeMigrator() -> DatabaseMigrator {
        ...
        // --- Fork sync (upstream/main, 2026-09) ---
        // Both sides independently numbered their next migrations from v42. Migration NAMES are the
        // real identifier GRDB persists per-device (renaming one would make it look unapplied to an
        // existing install and re-run against a database that already has the shape) — so these keep
        // their literal upstream names verbatim rather than being renumbered to look sequential; the
        // repeated "v42" here is a merge artifact, not a bug. Kept: everything upstream added in this
        // range except v46-lift-log, dropped deliberately — this fork already has its own strength-
        // training feature (v43/v44/v45/v46 above: strengthSession/strengthSet/strengthTemplate) built
        // independently of upstream's parallel liftSession/liftSet/liftProgram tables; adopting both
        // would mean two competing, unconnected ways to log a lift.
