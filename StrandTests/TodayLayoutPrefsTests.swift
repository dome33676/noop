...
final class TodayLayoutPrefsTests: XCTestCase {
    ...
    func testEncodeDecodeRoundTripsAReorderedList() {
        let reordered: [TodaySection] = [
            ...
            .stressMonitor, .journal, .menstrualCycle, .addedCards,
        ]
        let encoded = TodayLayoutPrefs.encode(reordered)
        XCTAssertEqual(encoded, "heartRate,hero,yourCards,liveSession,synthesis,keyMetrics,workouts,recoveryVitals,stressMonitor,journal,menstrualCycle,addedCards")
    ...
    /// them to the bottom of the user's saved order. Also covers stressMonitor, added later still, landing
    /// at its own default position (right after recoveryVitals, before yourCards).
    func testSavedOrderFromFirstCutInsertsHeroAndSessionAtTheirDefaultPosition() {
        ...
        XCTAssertEqual(
            ...
            // journal(9) follows everything saved → appended; addedCards(11) is last, appended after it.
            [.hero, .liveSession, .synthesis, .keyMetrics, .workouts, .heartRate, .recoveryVitals, .stressMonitor, .yourCards, .menstrualCycle, .journal, .addedCards]
        )
    }
    ...
    func testInsertsAnyMissingSectionAtItsDefaultPositionRelativeToSaved() {
        ...
        XCTAssertEqual(
            ...
            [.hero, .liveSession, .workouts, .heartRate, .synthesis, .keyMetrics, .recoveryVitals, .stressMonitor, .yourCards, .menstrualCycle, .journal, .addedCards]
        )
    }
    ...
    func testDropsUnknownTokensAndCollapsesDuplicates() {
        ...
        XCTAssertEqual(
            ...
            [.hero, .liveSession, .synthesis, .keyMetrics, .workouts, .recoveryVitals, .stressMonitor, .yourCards, .heartRate, .menstrualCycle, .journal, .addedCards]
        )
    }
    ...
    func testVisibleOrderFiltersHiddenWithoutChangingSavedOrder() {
        ...
        XCTAssertEqual(
            ...
            [.heartRate, .stressMonitor, .yourCards, .liveSession, .synthesis, .keyMetrics, .recoveryVitals, .menstrualCycle, .journal, .addedCards]
        )
        XCTAssertEqual(TodayLayoutPrefs.decodeOrder(order), [
            .heartRate, .hero, .stressMonitor, .yourCards, .liveSession, .synthesis, .keyMetrics, .workouts,
