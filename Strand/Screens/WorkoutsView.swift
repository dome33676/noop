...
struct WorkoutsView: View {
    ...
    /// state, #459 — including this fork's own "My Templates" extension, see WorkoutStartControl.swift)
    /// is isolated into `WorkoutStartControl`, mirroring `HealthView`'s live-observing-leaf pattern
    /// (HealthView.swift:17-22, 44-46), so a tick re-renders only that small leaf.
    ...
    var body: some View {
        ...
        // moved to `WorkoutStartControl` (including this fork's own "My Templates" extension there),
        // which owns `AppModel` itself — see the comment on this screen's `profile`/`intelligence`
        // properties above for why `model` can't live here.
