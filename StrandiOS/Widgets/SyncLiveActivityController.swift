...
///
/// Modelled on `LiveActivityController` (live HR), and separate from it for the same reason the two
/// activity types are separate. Attached once at launch to `LiveState`, rather than as a modifier on
/// `StrandiOSApp.body` (that chain is past the type-checker's budget, and this must also run in a
/// process the Sync Strap shortcut launched with no scene at all).
///
